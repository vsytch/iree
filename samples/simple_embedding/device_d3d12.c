...
// A example of setting up the the d3d12 driver.
...
#include "samples/simple_embedding/simple_embedding_test_bytecode_module_d3d12_c.h"

iree_status_t create_sample_device(iree_allocator_t host_allocator,
                                   iree_hal_device_t** out_device) {
  // Only register the D3D12 HAL driver.
  IREE_RETURN_IF_ERROR(iree_hal_d3d12_driver_module_register(
      iree_hal_driver_registry_default()));

  // Create the HAL driver from the name.
  iree_hal_driver_t* driver = NULL;
  iree_string_view_t identifier = iree_make_cstring_view("d3d12");
...
const iree_const_byte_span_t load_bytecode_module_data() {
  const struct iree_file_toc_t* module_file_toc =
      iree_samples_simple_embedding_test_module_d3d12_create();
